...
# Function to convert string (hex or ASCII) to list of bits
...
if __name__ == "__main__":
    ...
    bits = str_to_bits(sample_hex, is_hex=True)
    ip = initial_permutation(bits)
    print("Initial Permutation (Hex) of {}: {}".format(sample_hex, bits_to_hex(ip)))
    
    # ASCII input example
    sample_ascii = "ABCDEFGH"
    bits = str_to_bits(sample_ascii)
    ip = initial_permutation(bits)
    print("Initial Permutation (Hex) of '{}': {}".format(sample_ascii, bits_to_hex(ip)))
